...
Stop hook は `phase="eval"` のときだけ判定します。基準承認直後や実装途中 (`phase="implementing"`) は差し戻さず、`fable_review` が採点を書き込んだ後にだけ続行/終了を判断します。
...
リリース前の実走確認では、Codexの新規スレッドで `fable_plan → criteria提示 → fable_loop_approve → 実装 → fable_review → Stop hook` を1回通してください。これは実Fable呼び出しなので API コストが発生します。
