...
3. Claude Code CLI (`claude`) がなければ、Fableを呼ぶ必須ランタイムとして自動インストール
4. `AskUserQuestion` で **Claudeアカウントログイン（推奨）**、**Anthropic APIキー（従量課金）** の順に選択肢を表示し、選ばれた認証を完了
5. Fableの最小応答確認に成功してから、最後に「Fableの状態を確認して」と試せる状態にする

このリポジトリは Codex Plugin、Cursor Plugin、Antigravity Plugin を同梱します。ただし、セットアップ時に他クライアントへ勝手に横展開しません。Claude Code CLI は Fable を headless で呼ぶ必須ランタイムですが、Claude Codeという別のホストエージェントをセットアップするものではありません。
...
installer は Node.js / Codex CLIを確認し、Claude Code CLIがなければ自動導入してCodex Pluginを追加します。その後、**Claudeアカウントログイン（1番・推奨）**、**Anthropic APIキー（2番・従量課金）** の順で認証方式を案内し、実際のFable最小応答に成功した場合だけ完了します。AIエージェントにセットアップを頼んだ場合は、通常のチャット質問ではなく `AskUserQuestion` の選択UIを使います。APIキーは非表示入力で扱い、プロセス引数やログには出しません。**Codex アプリを閉じてから installer を実行**し、終わったら再起動して `Fableの状態を確認して` と送ります。

認証確認の最後にFableへ1回だけ最小リクエストを送ります。APIキー方式では少額の従量課金が発生します。認証状態だけ確認して実リクエストを省略する場合は `--skip-live-check`（PowerShellは `-SkipLiveCheck`）、意図的に未認証のままpluginだけ入れる場合は `--skip-auth`（PowerShellは `-SkipAuth`）を指定します。
...
あなた ──> Codex / Cursor / Antigravity (実装役)
              │  設計が必要なとき MCP ツールを呼ぶ
              ▼
          fable-mcp ──> claude -p --model <指定モデル / デフォルトFable 5>
                            │  リポジトリを探索して深く推論
                            ▼
...
デフォルトは `claude-fable-5` です。`fable_plan` / `fable_ask` / `fable_review` は呼び出しごとに `model` と `effort` を受け取ります。

- モデルは固定リストではありません。Claude Codeが受け付けるエイリアス、完全モデルID、将来追加されるモデルIDをそのまま指定できます。
- 例: `claude-opus-4-8`、`claude-opus-4-7`、`claude-opus-4-6`、`claude-sonnet-5`、`opus`、`sonnet`。Bedrock/Vertex形式の安全なモデルIDも通します。
- effortの完全な値は `low` / `medium` / `high` / `xhigh` / `max` です。モデルが対応しない組み合わせは、勝手に弱い設定へ落とさずClaude CLIのエラーを返します。
- 優先順位は `呼び出しごとのmodel > FABLE_MODEL > claude-fable-5`、`呼び出しごとのeffort > FABLE_EFFORT > モデルのデフォルト` です。
- 品質ループでは、plan時のモデルをreviewでも自動的に維持します。

例: 「Opus 4.8をxhighで」「Sonnet 5をmediumで」と頼むと、ホストエージェントは次の引数を渡します。
...
- Node.js 18+
- Claude Code CLI（Fable呼び出し用の必須ランタイム。標準installerでは不足時に自動導入）
...
Claude Code CLI はFable呼び出し用ランタイムとして、不足時に自動インストールします。続けてAskUserQuestionでClaudeアカウントログイン（先頭・推奨）またはAPIキーを選び、実呼び出しで利用可能性を検証します。
...
| `FABLE_MODEL` | `claude-fable-5` | 未指定時のモデル。各ツールの`model`引数で呼び出しごとに上書き可能。Claude Code対応の現行・将来モデルIDを固定リストなしで受け付ける |
