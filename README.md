...
一番簡単なセットアップ方法は、Codex や Claude Code などのAIエージェントに次の一文だけ貼ることです。GitHub を clone したり、MCP を手入力したりする必要はありません。

```text
https://github.com/taiyuhiga/fable-mcp をセットアップして。
```

この一文を受け取ったAIエージェントは、このREADMEとリポジトリ内の `AGENTS.md` をセットアップ契約として読み、要約で止まらず次を最後まで実行してください。

1. Node.js 18+ があるか確認
2. Claude Code CLI がなければ `npm i -g @anthropic-ai/claude-code` でインストール
3. このリポジトリを clone して `python3 setup-all.py` を実行
4. 使えるクライアントをまとめてセットアップ
   - Codex: Codex Plugin
   - Claude Code: Claude Code Plugin
   - Cursor: Cursor local plugin
   - Antigravity: Antigravity local plugin
5. `ANTHROPIC_API_KEY` が必要なところだけ私に聞く
6. 最後に「Fableの状態を確認して」と試せる状態にする

Claude Code CLI は Fable 5 を headless plan mode で起動するためのローカルランナーです。Cursor / Codex / Antigravity から使う場合でも内部では必要です。

ターミナルで自分で実行したい場合は、clone 後にこれを使います。
...
`setup-all.py` は Node.js / Claude Code CLI を確認し、インストール可能なクライアントへ plugin を配置します。API キーは対話で聞かれたときだけ貼ってください。終わったら該当アプリを再起動して `Fableの状態を確認して` と送ります。
...
v0.6 以降は **Codex plugin として入れるのが推奨**です。MCP 定義、品質ループの Stop hook、UserPromptSubmit watchdog、SessionStart でのルーティング指示注入を plugin に同梱しています。

Cursor / Claude Code / Antigravity も使う場合は、上の `python3 setup-all.py` が最短です。各クライアントでの配布形式は次の通りです。
...
Cursor Marketplace へ公開掲載する場合は、Cursor側のレビュー提出が別途必要です。このリポジトリには `.cursor-plugin/marketplace.json` と `plugins/fable-mcp-cursor/` を同梱しているため、ローカル/チーム配布から始められます。
...
### 手動設定から plugin へ移行する場合

過去に手動MCPで入れていた場合は、plugin 導入後に重複登録を消してください。残っていると **MCPサーバー二重起動** や **Stop hook 二重発火** が起きます。
...
この一文だけで、README冒頭と `AGENTS.md` の自動セットアップ契約に従って、Node.js確認、Claude Code CLI確認/インストール、Codex Plugin install、`codex mcp list` 確認まで進める想定です。`ANTHROPIC_API_KEY` が必要な場合だけ、エージェントがユーザーに聞きます。
